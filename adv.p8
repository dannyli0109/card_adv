...
function damage(target, dmg)
		if (target.block >= dmg) then
			target.block -= dmg
			add_float(dmg,target.x + target.w / 2 - #tostr(dmg)*2,target.y - 5,12)
		else 
			local block_reduce = target.block
			dmg -= target.block
			target.block = 0
			target.hp = max(0, target.hp
			 - dmg)
			
			if (block_reduce > 0) then
				add_float(block_reduce,target.x + target.w / 2 - #tostr(dmg)*2,target.y - 5,12)
				add_float(dmg,target.x + target.w / 2 - #tostr(dmg)*2,target.y - 5 - 8,8)
			else
				add_float(dmg,target.x + target.w / 2 - #tostr(dmg)*2,target.y - 5,8)
			end
		end
...
function pure_damage(target, dmg)
	...
	add_float(dmg,target.x + target.w / 2 - 1,target.y - 5,14)
	shake = 1
	sfx(1)
end
...
function block(target, amt)
	...
	for i=1,#tostr(player.block - amt)-1 do
		space = space.." "
	end

	space = space.."+"
	add_float(space..amt,0,6+10,11)
	sfx(2)
end
...
function compute_skills(s1,s2,mobs)
...
function draw_discard()
	local cards = player.discard
	for i=1,#cards do
		spr(81, player.discard_pos_x, player.discard_pos_y-8-flr((i-1)/2))
	end
end
...
function create_game()
	...
	g.state = create_main_menu_state()
	g.state.enter()
	g.draw = function()
		g.state.draw()
...
function create_mob(_typ,_x,_y,_w,_h)
	...
	mob.ani_percentage = 1
	mob.ani_increments = 0.08
	...
	mob.update = function()
		...
		if (mob.ani_percentage < 0.5) then
			mob.x = mob.x * mob.ani_percentage + (1-mob.ani_percentage) * (mob.sox)
			mob.y = mob.y * mob.ani_percentage + (1-mob.ani_percentage) * (mob.soy)
		else
			mob.x = mob.x * (1-mob.ani_percentage) + (mob.ani_percentage) * (mob.sox)
			mob.y = mob.y * (1-mob.ani_percentage) + (mob.ani_percentage) * (mob.soy)
		end
		a = 	mob.ani_percentage
		mob.ani_percentage = min(mob.ani_percentage + mob.ani_increments, 1)
	end
	...
	mob.animate = function(sox,soy,ox,oy,speed,cb)
		mob.ani_percentage = 0
		mob.ani_increments = speed
		mob.sox = sox
		mob.soy = soy
		mob.ox = ox
		mob.oy = oy
	...
	p.draw_card = function()
		...
		if #p.deck == 0 then
			for i=1,#p.discard do
				local card = p.discard[i]
				card.x = 0
				card.y = 0
				add(p.deck,card)
			end
			p.discard = {}
			update_hand_pos(16, 128-24,96,32,24,32)p.discard = {}
		end
...
function create_battle_state()
	...
	s.enter = function()
		s.fadeperc = 1
			for i=1,#player.hand do
				add(player.deck, player.hand[i])
			end
			
			for i=1,#player.discard do
				add(player.deck, player.discard[i])
			end
			...
			player.draw_cards(player.card_draws),
			function()
	...
	s.draw = function()
		...
		--update_hand_pos(16, 128-36,96,32,24,32)
		print("♥",0,0,8)
		print(player.hp.."/"..player.hp_max,12,0,7)
		pal(7,12)
		sspr(0,40,7,5,0,6)
		pal()
...
function select_monster_input_handler(s)
	return function()
		...
		if (btnp(5)) then
			...
			if (c_card_target[combinded_card] == "self") then return end
			
			local mobs = {}
			if (c_card_target[combinded_card] == "enemy") then
				add(mobs, s.mobs[player.selected_mob])
...
function do_skill(s,mobs)
	...
	add(sequence, function()
		...
		if (#s.mobs <= 0) then
			player.selects = {}
			player.selected_mob = 0
			game.state = create_selection_state()
			game.state.enter()
			s.co = {}
			return
		end
	...
	s.co = create_sequence(sequence, 20, function()
		if #player.hand >=2 then
			player.target_card(1)
			s.handle_input = select_card_input_handler(s)
		else
			local new_sequence = {}
			add(new_sequence, function()
				for i=1,#player.hand do
					player.discard_card(player.hand[i])
					add_float("not enouth cards", 32, 63,8)	
				end
			end)
			add(new_sequence, function()
				player.target = 0
				...
				for i=1,#player.hand do
					del(player.hand, player.hand[i])
				end
			end)
			
			s.co = create_sequence(new_sequence, 10, function()
					s.handle_input = function() end
					mobs_attack(s)
			end) 
			--shake = 1
		end
...
function mobs_attack(s)	
	...
	for i=1,#s.mobs do
		...
		add(sequence, function()
			...
			damage(player, mob.attack)
		end
		)
	end
	
		s.co = create_sequence(sequence, 40, function()
		s.co = create_sequence({
			function()
				if player.hp <= 0 then
					game.state = create_game_over_state()
					game.state.enter()
					s.co = {}
					return
				end	
...
function create_game_over_state()
	local s = {}
	s.wind = {}
	s.fadeperc = 1
	...
	s.handle_input = function()
		if btnp(5) then
			start_game()
			game.state = create_adventure_state()
			game.state.enter()
		end
	end
...
function create_sequence(arr,speed,cb)
	local c = {}
	c.speed = speed or 4
	c.f = c.speed
	c.cb = cb or function() end
...
function do_shake()
	local shake_x = 8 - rnd(16)
	local shake_y = 8 - rnd(16)
...

...

